...
public class FloatScalarHandler extends ScalarHandler<Float> {
   ...
   */
  public FloatScalarHandler(int columnIndex) {
   ...
   */
  public FloatScalarHandler(String columnName) {
    this(1, columnName);
  }
   ...
   * @param columnName The name of the column to retrieve from the <code>ResultSet</code>.
   */
  private FloatScalarHandler(int columnIndex, String columnName) {
    super();
  ...
  public Float handle(ResultSet rs) throws SQLException {
    ...
    if (rs.next()) {
      if (this.columnName == null) {
        return ((Number) rs.getObject(this.columnIndex)).floatValue();
      }
      return ((Number) rs.getObject(this.columnIndex)).floatValue();
    }
    return null;
