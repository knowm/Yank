...
public class IntegerScalarHandler extends ScalarHandler<Integer> {
   ...
   * @param columnName The name of the column to retrieve from the <code>ResultSet</code>.
   */
  private IntegerScalarHandler(int columnIndex, String columnName) {
    super();
  ...
  public Integer handle(ResultSet rs) throws SQLException {
    ...
    if (rs.next()) {
      if (this.columnName == null) {
        return ((Number) rs.getObject(this.columnIndex)).intValue();
      }
      return ((Number) rs.getObject(this.columnIndex)).intValue();
    }
    return null;
